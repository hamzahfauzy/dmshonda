<!DOCTYPE html>
<html>
<head>
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Z Framework</title>
	<link rel="stylesheet" type="text/css" href="<?=$this->assets->get('css/bootstrap.css')?>">
	<link rel="stylesheet" type="text/css" href="<?=$this->assets->get('fontawesome/css/all.css')?>">
	<link href="<?=$this->assets->get("mdb/css/bootstrap.min.css") ?>" rel="stylesheet">
	<!-- Material Design Bootstrap -->
	<link href="<?=$this->assets->get("mdb/css/mdb.min.css") ?>" rel="stylesheet">
	<!-- Your custom styles (optional) -->
	<link rel="stylesheet" type="text/css" href="<?=$this->assets->get('css/styles.css')?>">

	<!-- JQUERY -->
	<script src="<?= $this->assets->get('js/jquery.js') ?>"></script>
	<script src="<?= $this->assets->get('js/bootstrap.js') ?>"></script>
	<script src="<?=$this->assets->get('js/site.js')?>"></script>
</head>
<body>